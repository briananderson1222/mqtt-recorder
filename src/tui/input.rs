...
/// Handle Ctrl+C / 'q': request application quit.
fn handle_quit(state: &Arc<TuiState>, shutdown_tx: &broadcast::Sender<()>) -> KeyAction {
    state.request_quit();
    let _ = shutdown_tx.send(());
    KeyAction::Quit
}

/// Handle 'm': toggle mirroring on/off, logging session stats.
fn handle_toggle_mirror(state: &Arc<TuiState>) -> KeyAction {
...
/// Handle 'r': toggle recording on/off. Recording and playback are mutually exclusive.
fn handle_toggle_record(state: &Arc<TuiState>) -> KeyAction {
...
/// Handle 's': toggle the MQTT source connection on/off.
fn handle_toggle_source(state: &Arc<TuiState>) -> KeyAction {
...
/// Handle 'f': enter input mode to select the recording file.
fn handle_file_selection() -> KeyAction {
...
/// Handle 'A': enter input mode to select the audit log file.
fn handle_audit_file_selection(state: &Arc<TuiState>) -> KeyAction {
    KeyAction::EnterInputMode {
        target_audit: true,
...
/// Handle 'p': toggle playback on/off (start, restart, or stop depending on current state).
fn handle_toggle_playback(state: &Arc<TuiState>) -> KeyAction {
            ...
            if is_on && is_finished {
                ...
                if state.is_recording() {
                    ...
                    state.push_audit(
                        ...
                        AuditSeverity::Info,
                        format!(
                            "Recording OFF ({} this session{}, {} total) — playback enabled",
                            session, dur_str, total
                        ),
                    );
                }
...
/// Handle 'l': toggle loop mode for playback.
fn handle_toggle_loop_mode(state: &Arc<TuiState>) -> KeyAction {
...
/// Handle '+'/'=' and '-': cycle the playback speed up or down.
fn handle_speed_key(state: &Arc<TuiState>, increase: bool) -> KeyAction {
    let speed = state.cycle_playback_speed(increase);
    state.push_audit(
        AuditArea::Playback,
        AuditSeverity::Info,
        format!(
...
/// Handle Up/Down arrows: navigate the file selection list.
fn handle_navigate_selection(state: &Arc<TuiState>, delta: isize) -> KeyAction {
    state.navigate_selection(delta);
    KeyAction::None
}

/// Handle Enter: confirm the currently selected file/menu item.
fn handle_confirm_selection(state: &Arc<TuiState>) -> KeyAction {
    state.confirm_selection();
    KeyAction::None
}

/// Handle 'j': scroll the audit log down.
...
/// Handle 'k': scroll the audit log up.
fn handle_scroll_up(log_scroll: &mut usize, log_pinned_bottom: &mut bool) -> KeyAction {
...
/// Handle 'a': toggle the audit log panel visibility.
fn handle_toggle_audit(state: &Arc<TuiState>) -> KeyAction {
    state.toggle_audit();
    KeyAction::None
}
...
fn handle_normal_key(
    ...
    match key.code {
        ...
        {
            handle_quit(state, shutdown_tx)
        }
        KeyCode::Char('q') => handle_quit(state, shutdown_tx),
        KeyCode::Char('m') => handle_toggle_mirror(state),
        KeyCode::Char('r') => handle_toggle_record(state),
        KeyCode::Char('s') => handle_toggle_source(state),
        KeyCode::Char('f') => handle_file_selection(),
        KeyCode::Char('A') => handle_audit_file_selection(state),
        KeyCode::Char('p') => handle_toggle_playback(state),
        KeyCode::Char('l') => handle_toggle_loop_mode(state),
        KeyCode::Char('+') | KeyCode::Char('=') => handle_speed_key(state, true),
        KeyCode::Char('-') => handle_speed_key(state, false),
        KeyCode::Up => handle_navigate_selection(state, -1),
        KeyCode::Down => handle_navigate_selection(state, 1),
        KeyCode::Enter => handle_confirm_selection(state),
        KeyCode::Char('j') => handle_scroll_down(state, log_scroll, log_pinned_bottom),
        KeyCode::Char('k') => handle_scroll_up(log_scroll, log_pinned_bottom),
        KeyCode::Char('a') => handle_toggle_audit(state),
        _ => KeyAction::None,
    }
}
